...
/*                                                              */
/*    NAME: ODSGR10                                             */
/*   TITLE: Modifying Tick Marks and Suppressing Grid Lines     */
/* PRODUCT: SAS                                                 */
/*  SYSTEM: ALL                                                 */
/*    KEYS: ODS Graphics, modifying tick marks, grid lines      */
/*   PROCS:                                                     */
/*    DATA:                                                     */
/*                                                              */
/*     REF:                                                     */
...
layout Gridded;
   ...
   EndLayout;
   layout Overlay /
      xaxisopts=(label=_DEPLABEL display=standard)
      yaxisopts=(label="Normal Quantile" display=all
                 linearopts=(tickvaluelist=(-3 -2 -1 0 1 2)));
      SCATTERPLOT x=eval(SORT(DROPMISSING(RESIDUAL)))
      ...
      lineparm slope=eval(1/STDDEV(RESIDUAL))
         Y=eval(-MEAN(RESIDUAL)/STDDEV(RESIDUAL)) /
         lineattrs=(color=red
                    pattern=dash
