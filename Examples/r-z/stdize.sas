 ...
 /*           new macro %in operator caused error 29Apr02        */
 /*           when METHOD=IN                                     */
 /*           added +1 and -1 to the boundary vlaues for AWAVE   */
 /*              function (see S0176684).          08Jan03       */
 /*           changed fuzzing condition (S0376860) 19Nov06       */
 /*                                                              */
 /****************************************************************/
...
separated by commas:
   ...
   FUZZ=        Relative fuzz factor. Default is 1E-14.
                If abs(score) < mult * fuzz then score = 0;
                If scale < abs(location) * fuzz then scale = 0;
                else If abs(location) < mult * fuzz then location = 0;
...
%macro xstdize(data=,method=,var=,replace=0,reponly=0,missing=,
   fuzz=1e-14,initial=MAD,vardef=DF,pctldef=5,add=0,mult=1,
      ...
      %do i=1 %to &nvar;
         ...
         %if ^&nostd %then %do;
            ...
            %if %bquote(&fuzz)^=0 %then %do;
               if abs(&&var&i)<=(&mult)*(&fuzz) then &&var&i=0;
            %end;
         %end;
         %if ^&missbit %then %do;
            end;
   ...
   %if &estbit %then %if %bquote(&fuzz)^=0 %then %do;
      ...
      data &instat;
         ...
         %do i=1 %to &nvar;
            ...
            else if abs(&&loc&i)<= (&mult)*(&fuzz) then &&loc&i=0;
         %end;
      run;
      %if &syserr>4 %then %goto fail;
      options _last_=&_out;
...
%if %xsubstr(&method,1,2)=%quote(IN) %then %let method= STD_&method;
%if %scan(&method,1)=L %then %let method=STD_&method;

%let initial=%upcase(&initial);
%if %xsubstr(&initial,1,2)=%quote(IN) %then %let initial= STD_&initial;
